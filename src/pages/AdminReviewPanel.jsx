...
export default function AdminReviewPanel() {
  ...
  async function saveQuestion(idx) {
    ...
    try {
      ...
      const qlist = [...questions];
      qlist[idx] = { ...updated, editing: false };
      setQuestions(qlist);
      setMessage("Saved");
    } catch (e) {
      console.error(e);
  ...
  function toggleAccept(idx) {
    const qlist = [...questions];
    qlist[idx].accepted = !qlist[idx].accepted;
    setQuestions(qlist);
  }
  ...
  async function handleReplaceImage(idx, fileInput) {
    ...
    if (!fileInput.files || fileInput.files.length === 0)
      return setMessage("Choose an image file");
    const fd = new FormData();
    fd.append("image", fileInput.files[0]);
    try {
      setMessage("Uploading image...");
      const res = await fetch(`${API}/api/admin/questions/${q.id}/image`, {
        method: "POST",
        body: fd,
      });
      const data = await res.json();
        ...
        <div className="bg-white p-4 rounded shadow mb-6">
          <form onSubmit={handleUpload} className="flex gap-3 items-center">
            ...
            <div className="text-sm text-gray-600 ml-4">
              Tip: Use high-quality PDF for best extraction.
            </div>
          </form>
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="col-span-1 bg-white p-4 rounded shadow h-[60vh] overflow-auto">
            ...
            <ul>
              {docs.map((doc) => (
                <li
                  key={doc.id}
                  className={`p-2 rounded hover:bg-gray-50 cursor-pointer ${selectedDoc && selectedDoc.id === doc.id ? "bg-gray-100" : ""}`}
          ...
          <div className="col-span-2 bg-white p-4 rounded shadow h-[60vh] overflow-auto">
            <h2 className="font-semibold mb-3">
              {selectedDoc
                ? `Review: ${selectedDoc.filename}`
                : "Select a document to review"}
            </h2>

            {!selectedDoc && (
              <div className="text-sm text-gray-600">
                Click a document on the left to load parsed questions and
                images.
              </div>
            )}
            ...
            {selectedDoc && (
              <div>
                <div className="mb-3 text-sm text-gray-500">
                  Document ID: {selectedDoc.id} &nbsp; | &nbsp; Uploaded:{" "}
                        ...
                        <div className="w-48">
                            ...
                            {q.QuestionImages && q.QuestionImages.length > 0 ? (
                              <div className="space-y-2">
                                {q.QuestionImages.map((img) => (
                                  <img
                                    key={img.id || img.gcsUri}
                                    src={img.gcsUri}
                                    alt="diagram"
                                    className="w-full h-28 object-contain border"
                                  />
                                ))}
                              </div>
                            ) : (
                              <div className="w-full h-28 flex items-center justify-center bg-gray-50 border text-xs text-gray-400">
                                No image
                              </div>
                            )}
                          </div>
                          ...
                          <div className="mt-2 space-y-2">
                            ...
                            <input
                              ...
                              accept="image/*"
                              onChange={(e) =>
                                handleReplaceImage(idx, e.target)
                              }
                              className="text-xs"
                            />
                          </div>
                          ...
                          <div className="mt-2 flex gap-2">
                            <button
                              ...
                              className={`px-2 py-1 rounded text-xs ${q.accepted ? "bg-green-600 text-white" : "bg-gray-100"}`}
                            >
                            ...
                            <button
                              onClick={() => {
                                const copy = [...questions];
                                copy[idx].rejected = true;
                                setQuestions(copy);
                              }}
                              className={`px-2 py-1 rounded text-xs ${q.rejected ? "bg-red-600 text-white" : "bg-gray-100"}`}
                            >
                              Reject
                            </button>
                          </div>
